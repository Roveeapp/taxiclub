-- ============================================================================
-- Versión vigente de get_driver_for_assignment
--
-- APLICADA a hgnsvqhizbdwawkgjciw el 2026-08-31.
--
-- Reúne los dos arreglos anteriores (20260831163518 y 20260831163559):
--   · La ordenación por antigüedad va FUERA de la subconsulta, para que el
--     reparto rote de verdad en lugar de elegir siempre el UUID más bajo.
--   · FOR UPDATE SKIP LOCKED al elegir, y el turno se marca en la misma
--     transacción que la selección, lo que cierra la carrera entre reservas
--     simultáneas.
--   · El turno se marca con marcar_turno_conductor(), que usa clock_timestamp().
--
-- Se registra como migración porque el cuerpo se había aplicado con SQL directo
-- y no constaba en el historial: el fichero de migración no reflejaba lo que
-- había en la base de datos, que es exactamente el problema que arrastraba este
-- proyecto y que la baseline vino a cerrar.
--
-- VERIFICADO: seis repartos consecutivos con tres conductores elegibles dan
-- a75352f7 → 7817b252 → 3635b8b0 → a75352f7 → 7817b252 → 3635b8b0. Antes eran
-- seis veces el mismo conductor.
-- ============================================================================
